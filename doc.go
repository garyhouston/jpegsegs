...
Writing a multi-image file with MPF requires that the file positions of all images be encoded into the MPF index. The approach taken here is to initially write the index into the first image with nominal values, to reserve the appropriate amount of space in the APP2 segment. After all images have been writen to the output, and the positions collected, the APP2 segment is then rewritten with the final positions. This is demonstrated by the jpegsegscopy program.
*/
package jpegsegs
